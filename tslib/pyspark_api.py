...
class TSAccessor:
    ...
    def tsset(self, ts_args: SparkOpts | dict):
        ...
        if start is not None:
            ...
            end = self._obj.agg(F.max(ts_column).alias("__min__")).toPandas()[
                "__min__"
            ][0]
    ...
    def tsfill(
        ...
        *,
        ...
        if not is_panel:
            out = full.join(ts.data, ts.ts_column_name, "left")
        else:
            ids = ts.data.select(ts.panel_column_name).distinct()
            full = full.crossJoin(ids)
            out = full.join(ts.data, [ts.ts_column_name, ts.panel_column_name], "left")
        return out
    ...
    def with_lag(
        ...
        *,
        ...
        if not ts.is_date:
            ...
            # we need to generate a rangeSpec based on a numeric version of the ts_column
            # and the freq.
            # easiest way to do this is to cast the ts_column to unix timestamp
            # and take the rangeSpec as (ts.complete_time_series[i] - ts.complete_time_series[i-1])
            dta = ts.data.withColumn("__timestamp__", F.unix_timestamp(ts.ts_column_name))
            order_column = "__timestamp__"
            complete_timestamp = ts.complete_time_series.astype("datetime64[ns]")
            offset = complete_timestamp[1] - complete_timestamp[0]
        if not is_panel:
            return dta.withColumn(
                col_name,
                F.max(col_name).over(
                    Window.orderBy(order_column).rangeBetween(
                        -1 * back * offset, -1 * back * offset
                    )
                ),
            ).drop("__timestamp__")
        else:
            return dta.withColumn(
                ...
                F.max(col_name).over(
                    ...
                    .rangeBetween(-1 * back * offset, -1 * back * offset)
                ),
            ).drop("__timestamp__")
